...
class OrderHealthChecker:
    ...
    async def _perform_health_check_internal(
        ...
        try:
            ...
            if missing_grids:
                # 显示缺失的网格详情
                # 🔥 修复：将 profit_gap_grids (set) 转换为排序列表
                profit_gap_list = sorted(
                    profit_gap_grids) if profit_gap_grids else []

                for grid_id in missing_grids[:5]:  # 最多显示5个
                    price = self.config.lower_price + \
                        (grid_id - 1) * self.config.grid_interval
                    # 判断是买单还是卖单
                    side_str = "BUY" if (
                        profit_gap_list and grid_id < profit_gap_list[0]) else "SELL"
                    self.logger.info(
                        f"      - Grid {grid_id} @ ${price:.1f} ({side_str})")
                if len(missing_grids) > 5:
                    self.logger.info(
                        f"      ... 还有 {len(missing_grids) - 5}个缺失网格")
            ...
            if has_order_issues:
                ...
                if order_count_abnormal:
                    issues.append(
                        f"缺{self.config.grid_count - first_order_count}个订单")
            ...
            if order_count_abnormal:
                ...
                self.logger.info(
                    f"    当前状态: {buy_count}买 + {sell_count}卖 = {first_order_count}个")
                self.logger.info(
                    f"    修复后: {expected_buy_count}买 + {expected_sell_count}卖 = {self.config.grid_count}个 (补充{self.config.grid_count - first_order_count}个{'买单' if self.config.grid_type in [GridType.LONG, GridType.FOLLOW_LONG, GridType.MARTINGALE_LONG] else '卖单'})")
            else:
                # 订单数量正确，基于当前实际买卖单计算
                expected_buy_count = buy_count
                expected_sell_count = sell_count
                ...
                self.logger.info(
                    f"    实际状态: {buy_count}买 + {sell_count}卖 = {self.config.grid_count}个")
            ...
            if position_abnormal:
                self.logger.info(
                    f"  ❌ 持仓健康: 异常（持仓{'多了' if actual_pos > expected_position else '少了'} {position_diff})")
            else:
            ...
            # 判断是否有问题订单需要清理
            has_problem_orders = (
                duplicate_count > 0 or out_range_count > 0 or off_grid_count > 0)
            ...
            if missing_grids and len(missing_grids) > 0:
                ...
                if is_scalping_active:
                    # 剥头皮模式激活，只报告缺失，不补单
                    self.logger.info(
                        f"    🔴 检测到 {len(missing_grids)}个缺失网格，剥头皮模式跳过补单")
                elif not allow_filling:
                    self.logger.info(
                        f"    ⚠️ 检测到 {len(missing_grids)}个缺失网格，订单数已达上限，禁止补单")
                else:
                    ...
                    for grid_id in missing_grids[:5]:
                        ...
                        side_str = "BUY" if (
                            profit_gap_list and grid_id < profit_gap_list[0]) else "SELL"
                        self.logger.info(
                            f"      ✅ Grid {grid_id} @ ${price:.1f} ({side_str} 0.002) - 已挂单")
                    if len(missing_grids) > 5:
                        self.logger.info(
                            f"      ... 还有 {len(missing_grids) - 5}个")
            else:
            ...
            if position_abnormal and not is_scalping_active:
                self.logger.info(
                    f"    🔧 执行持仓调整: {', '.join(position_health['issues'])}")
                self.logger.info(
            ...
            # 🔥 只有在系统稳定（无任何问题）时才进行缓存同步
            if has_any_issues:
                self.logger.info("    ⏸️  本轮检查发现问题，跳过缓存同步（等待下一轮）")
            elif is_scalping_active:
                self.logger.info("    ⏸️  剥头皮模式激活，跳过缓存同步")
            else:
                # 系统稳定，执行缓存同步
                self.logger.info("    ✅ 系统稳定，执行缓存同步")
                
                # 同步 _pending_orders 缓存
                ...
                self.logger.info(
                    f"    📊 缓存检查: 缓存={cache_count}个, 市场={market_count}个, "
                    f"差异={abs(cache_count - market_count)}个"
                )

                # 如果缓存数量不一致，清理 _pending_orders_by_client_id
                if cache_count != market_count:
                    self.logger.info("    🔧 检测到缓存差异，执行清理")
                    await self._clean_stale_client_id_cache(orders)
                else:
                    self.logger.info("    ✅ 缓存一致")
    ...
    async def _clean_stale_client_id_cache(self, exchange_orders: List):
        """
        清理 _pending_orders_by_client_id 中多余的缓存条目
        
        场景：
        - _pending_orders 已通过 _sync_orders_from_exchange 同步
        - 但 _pending_orders_by_client_id 中可能有已成交订单的 client_id 残留
        - 需要清理这些无效的 client_id 映射
        
        Args:
            exchange_orders: 交易所实际订单列表
        """
        try:
            # 构建有效的 order_id 集合
            valid_order_ids = {order.id for order in exchange_orders if order.id}
            
            # 遍历 client_id 缓存，删除无效条目
            removed_count = 0
            for client_id in list(self.engine._pending_orders_by_client_id.keys()):
                cached_order = self.engine._pending_orders_by_client_id[client_id]
                
                # 如果该订单的 order_id 不在有效集合中，删除缓存
                if cached_order.order_id not in valid_order_ids:
                    del self.engine._pending_orders_by_client_id[client_id]
                    removed_count += 1
                    self.logger.debug(
                        f"🗑️ 清理无效client_id缓存: {client_id[:10]}... (order_id={cached_order.order_id[:10] if cached_order.order_id else 'None'}...)"
                    )
            
            # 检查清理后的状态
            cache_count_after = len(self.engine._pending_orders_by_client_id)
            market_count = len(exchange_orders)
            
            if removed_count > 0:
                self.logger.info(
                    f"    ✅ 清理完成: 移除{removed_count}个无效client_id, "
                    f"清理后缓存={cache_count_after}个, 市场={market_count}个"
                )
            else:
                self.logger.debug("    ℹ️  未发现需要清理的client_id缓存")
                
            # 如果清理后仍有差异，记录警告
            if cache_count_after != market_count:
                self.logger.warning(
                    f"    ⚠️ 清理后仍有差异: 缓存={cache_count_after}个 vs 市场={market_count}个, "
                    f"差异={abs(cache_count_after - market_count)}个"
                )
                
        except Exception as e:
            self.logger.error(f"❌ 清理client_id缓存失败: {e}")
            import traceback
            self.logger.error(traceback.format_exc())
