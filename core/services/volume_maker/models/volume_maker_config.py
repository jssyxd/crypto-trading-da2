...
class VolumeMakerConfig:
    ...
    # 🔥 双交易所符号配置（用于Lighter刷量系统）
    signal_exchange: str = "backpack"  # 信号源交易所：backpack 或 hyperliquid
    # 信号源交易所符号（如 Backpack: "BTC_USDC_PERP"）
    signal_symbol: Optional[str] = None
    execution_symbol: Optional[str] = None   # 执行端交易所符号（如 Lighter: "BTC"）
    ...
    # 滑点配置（市价单专用）
    slippage: Decimal = Decimal("0.0002")  # 基础滑点（万分之2 = 0.02%）
    ...
    consecutive_fail_wait_minutes: int = 15  # 连续失败达到阈值后等待时间（分钟），0表示直接停止
    ...
    def from_dict(cls, data: dict) -> 'VolumeMakerConfig':
        ...
        config = cls(
            ...
            signal_exchange=vm_data.get('signal_exchange', 'backpack'),
            ...
            slippage=Decimal(str(vm_data.get('slippage', 0.0002))),
            ...
            consecutive_fail_wait_minutes=vm_data.get('consecutive_fail_wait_minutes', 15),
